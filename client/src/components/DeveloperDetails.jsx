...
import FollowingPostList from "./PostListFollowing";
...
let DeveloperDetails = () => {
  ...
  const fetchDeveloper = async () => {
    const { data } = await axios.get(`https://dev-room-backend.onrender.com/api/profiles/${developerId}`, {
      headers: {
  ...
  const UpdateFollowStatus = async () => {
    ...
    const { data: developerData } = await axios.get(`https://dev-room-backend.onrender.com/api/profiles/${developerId}`, {
      headers: {
        "Content-Type": "application/json",
      },
    });

    let { data: profileData } = await axios.get("https://dev-room-backend.onrender.com/api/profiles/me", {
      headers: {
        "Content-Type": "application/json",
    ...
    if (profileData && profileData.profile && developerData && developerData.profile) {
  ...
  const toggleFollowStatus = async () => {
    ...
    let response = await axios.put(`https://dev-room-backend.onrender.com/api/profiles/follow/${developerData.profile.user._id}`,
      {},
      {
        headers: {
          "Content-Type": "application/json",
...

                      {
                        isUser ?
                          ...
                          <div onClick={toggleFollowStatus}>
                            {
                              following ?
                                <div className="btn btn-teal btn-sm me-2">UNFOLLOW</div > : <div className="btn btn-teal btn-sm me-2">FOLLOW</div >
                            }
                            <div className="btn btn-teal btn-sm me-2 mt-2">FOLLOWER : {selectedProfile.followers.length}</div>
                            <Link to="/posts/FPost" >
                              <span className="btn btn-teal btn-sm me-2 hovertext" data-hover="Click to Show Following Post" >FOLLOWING : {selectedProfile.following.length}</span>
                            </Link>
                          </div>
