...
let Navbar = () => {
  ...
  const getUser = async() =>{
    const { data } = await axios.get("https://dev-room-backend.onrender.com/api/users/me", {
      headers: {
 ...
 if(localStorage.getItem("devroom"))
  ...
  let afterLogin = (
    <React.Fragment>
      ...
      <li className="nav-item">
        <Link to="/posts/FPost" className="nav-link">
          <i className="fa fa-list" /> Following Posts
        </Link>
      </li>
      <li className="nav-item">
