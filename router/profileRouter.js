...
/*
    @usage : Follow/Unfollow a user with userId
    @url : /api/profiles/follow/:userId_of_2nd_person
    @fields : no fields
    @method : PUT
    @access : PRIVATE
 */
router.put("/follow/:userId_of_2nd_person", authenticate, async (request, response) => {
  try {

    let userId_of_2nd_person = request.params.userId_of_2nd_person;
    var userId = request.user.id;

    // check if profile exists
    let profileUser = await Profile.findOne({ user: userId });
    let profile_2ndUser = await Profile.findOne({ user: userId_of_2nd_person });
    console.log("logging profile hehe: \n", profileUser, "\n", profile_2ndUser);

    if (!profileUser || !profile_2ndUser) {
      return response
        .status(401)
        .json({ errors: [{ msg: "No Profile Found" }] });
    }

    var isFollowed = profileUser.following.includes(userId_of_2nd_person);

    var option = isFollowed ? "$pull" : "$addToSet";

    // update to db
    let profile1st = await Profile.findOneAndUpdate(
      { user: userId },
      {
        [option]: { following: userId_of_2nd_person },
      },
      { new: true }
    ).populate("user", ["_id", "avatar"]);

    let profile2nd = await Profile.findOneAndUpdate(
      { user: userId_of_2nd_person },
      {
    ...
    response.status(200).json({
      ...
      profile1st: profile1st,
      profile2nd: profile2nd
    });
  } catch (error) {
    console.error(error);
    response.status(500).json({ errors: [{ msg: error.message }] });
